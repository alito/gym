...
class PassthroughController(object):
    ...
    def __init__(self, game_name, width, height, buttons_used, args):
        ...
        socket_path = args
        self.gym_connection = Socket()
        self.gym_connection.start_client(socket_path)

        # send dimensions
        self.gym_connection.send("size %sx%s\n" % (self.width, self.height))
        self.gym_connection.send("used %s\n" % (''.join('1' if used else '0' for used in self.buttons_used)))
    ...
    def update(self, score, game_over, video_frame):
        ...
        if frames_to_skip < 0:
            frames_to_skip = 0
            self.gym_connection.send('updt %d\n%d\n%s' % (score, game_over, video_frame))
    ...
    def receive_message(self):
        try:
            ...
            command = self.gym_connection.receive_bytes(4).lower()
            if command == 'inpt':
                # we have the size of the space. Initialise buffer and observation space
                input_description = self.gym_connection.receive_until_character('\n').strip()
                self._set_input(input_description)
            elif command == 'rest':
                # reset
                self.we_should_reset = True
            elif command == 'skip':
                skip_description = self.gym_connection.receive_until_character('\n').strip()
